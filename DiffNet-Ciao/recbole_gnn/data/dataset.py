...
class CIRecDataset(SocialDataset):
    ...
    def get_treatment(self, data_inter, data_trust):
        ...
        for user_id in data_inter['user_id'].drop_duplicates():
            ...
            k += 1
        friend_user_result = list(itertools.chain.from_iterable(friend_user_result))  # 转换成一维列表
        friend_item_result = list(itertools.chain.from_iterable(friend_item_result))  # 转换成一维列表
        friend_item_result = list(itertools.chain.from_iterable(friend_item_result))  # 转换成一维列表
    ...
    def get_cf_data_user(self, data_user_embedding, adj, T_f):
        ...
        # node_pairs_user = np.unique(np.concatenate([np.array(adj.nonzero()).T, np.array(T_f.nonzero()).T], axis=0), axis=0)
        a_unique = np.array(adj.nonzero()).T
        b_unique = np.array(T_f.nonzero()).T
        intersection = np.intersect1d(a_unique.reshape(-1, a_unique.shape[-1])[:, 0], b_unique.reshape(-1, b_unique.shape[-1])[:, 0])
        node_pairs_user = a_unique[np.in1d(a_unique[:,0], intersection)]
