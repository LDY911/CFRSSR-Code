...
class MHCN(SocialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.cut_socail_network_s = config.cut_socail_network_s
        self.cut_socail_network_j = config.cut_socail_network_j
        self.file_type = config.file_type
        self.socail_network_improvement_s = config.socail_network_improvement_s
        self.socail_network_improvement_j = config.socail_network_improvement_j
        self.improvement_type = config.improvement_type
        self.weighted_type = config.weighted_type
        ...
        if config.socail_network_improvement_s:
            temp = torch.tensor(pickle.load(open('./中间数据/' + dataset.dataset_name + '_user_improvement_degree_' + config.thresh + '_' + str(config.improvement_type) + '_' + str(config.file_type) + '.pkl', 'rb'))).to(self.device)
            if config.weighted_type == 0:
                self.H_s_edge_weight = temp[self.H_s_edge_index[1]].to(torch.float32).to(self.device)
            if config.weighted_type == 1:
                self.H_s_edge_weight_new = temp[self.H_s_edge_index[1]].to(torch.float32).to(self.device)
                self.H_s_edge_weight = (self.H_s_edge_weight + self.H_s_edge_weight_new) / 2
            if config.weighted_type == 2:
                self.H_s_edge_weight_new = temp[self.H_s_edge_index[1]].to(torch.float32).to(self.device)
                self.H_s_edge_weight = self.H_s_edge_weight * self.H_s_edge_weight_new
                self.H_s_edge_weight = (self.H_s_edge_weight - self.H_s_edge_weight.min()) / (self.H_s_edge_weight.max() - self.H_s_edge_weight.min())
        self.H_j_edge_index, self.H_j_edge_weight = self.get_edge_index_weight(H_j)
        if config.socail_network_improvement_j:
            temp = torch.tensor(pickle.load(open('./中间数据/' + dataset.dataset_name + '_user_improvement_degree_' + config.thresh + '_' + str(config.improvement_type) + '_' + str(config.file_type) + '.pkl', 'rb'))).to(self.device)
            if config.weighted_type == 0:
                self.H_j_edge_weight = temp[self.H_j_edge_index[1]].to(torch.float32).to(self.device)
            if config.weighted_type == 1:
                self.H_j_edge_weight_new = temp[self.H_j_edge_index[1]].to(torch.float32).to(self.device)
                self.H_j_edge_weight = (self.H_j_edge_weight + self.H_j_edge_weight_new) / 2
            if config.weighted_type == 2:
                self.H_j_edge_weight_new = temp[self.H_j_edge_index[1]].to(torch.float32).to(self.device)
                self.H_j_edge_weight = self.H_j_edge_weight * self.H_j_edge_weight_new
                self.H_j_edge_weight = (self.H_j_edge_weight - self.H_j_edge_weight) / (self.H_j_edge_weight - self.H_j_edge_weight)
    ...
    def get_motif_adj_matrix(self, dataset):
        ...
        B = S.multiply(S.T)
        U = S - B
        if not self.cut_socail_network_s and not self.cut_socail_network_j:
            ...
            A9 = A9 + A9.T
            A10 = Y.dot(Y.T) - A8 - A9
        elif self.cut_socail_network_s and not self.cut_socail_network_j:
            S_cut = pickle.load(open('dataset/' + dataset.dataset_name + '/' + dataset.dataset_name + '_cut_net.pkl', 'rb'))
            B_cut = S_cut.multiply(S_cut.T)
            U_cut = S_cut - B_cut
            C1 = (U_cut.dot(U_cut)).multiply(U_cut.T)
            A1 = C1 + C1.T
            C2 = (B_cut.dot(U_cut)).multiply(U_cut.T) + (U_cut.dot(B_cut)).multiply(U_cut.T) + (U_cut.dot(U_cut)).multiply(B_cut)
            A2 = C2 + C2.T
            C3 = (B_cut.dot(B_cut)).multiply(U_cut) + (B_cut.dot(U_cut)).multiply(B_cut) + (U_cut.dot(B_cut)).multiply(B_cut)
            A3 = C3 + C3.T
            A4 = (B_cut.dot(B_cut)).multiply(B_cut)
            C5 = (U_cut.dot(U_cut)).multiply(U_cut) + (U_cut.dot(U_cut.T)).multiply(U_cut) + (U_cut.T.dot(U_cut)).multiply(U_cut)
            A5 = C5 + C5.T
            A6 = (U_cut.dot(B_cut)).multiply(U_cut) + (B_cut.dot(U_cut.T)).multiply(U_cut.T) + (U_cut.T.dot(U_cut)).multiply(B_cut)
            A7 = (U_cut.T.dot(B_cut)).multiply(U_cut.T) + (B_cut.dot(U_cut)).multiply(U_cut) + (U_cut.dot(U_cut.T)).multiply(B_cut)
            A8 = (Y.dot(Y.T)).multiply(B)
            A9 = (Y.dot(Y.T)).multiply(U)
            A9 = A9 + A9.T
            A10 = Y.dot(Y.T) - A8 - A9
        elif not self.cut_socail_network_s and self.cut_socail_network_j:
            S_cut = pickle.load(open('dataset/' + dataset.dataset_name + '/' + dataset.dataset_name + '_cut_net.pkl', 'rb'))
            B_cut = S_cut.multiply(S_cut.T)
            U_cut = S_cut - B
            C1 = (U.dot(U)).multiply(U.T)
            ...
            A6 = (U.dot(B)).multiply(U) + (B.dot(U.T)).multiply(U.T) + (U.T.dot(U)).multiply(B)
            A7 = (U.T.dot(B)).multiply(U.T) + (B.dot(U)).multiply(U) + (U.dot(U.T)).multiply(B)
            A8 = (Y.dot(Y.T)).multiply(B_cut)
            A9 = (Y.dot(Y.T)).multiply(U_cut)
            A9 = A9 + A9.T
            A10 = Y.dot(Y.T) - A8 - A9
        else:
            S_cut = pickle.load(open('dataset/' + dataset.dataset_name + '/' + dataset.dataset_name + '_cut_net.pkl', 'rb'))
