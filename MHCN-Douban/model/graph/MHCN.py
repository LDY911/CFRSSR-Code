...
class MHCN(GraphRecommender):
    def __init__(self, conf, training_set, test_set, **kwargs):
        ...
        self.weighted_type = int(args['-weighted_type'])
        self.file_type = args['-file_type']
        self.dataset_name = self.config['training.set'].split('dataset/')[1].split('/')[0]
        self.num_users = len(self.data.id2user)
        self.num_items = len(self.data.id2item)
        self.refactor_socail_network = True if args['-refactor_socail_network'] == 'True' else False
    ...
    def build_hyper_adj_mats(self):
        ...
        A9 = A9 + A9.T
        A10 = Y.dot(Y.T) - A8 - A9
        if self.refactor_socail_network:
            print('Reconstructing the social network:', self.thresh)
            temp = pd.read_csv('dataset/' + self.dataset_name + '/'+self.dataset_name + '_' + self.thresh + '_influenced_inter_data_j.csv')
            temp = temp.astype(str)
            temp['user_id'] = temp['user_id'].replace(self.data.user)
            temp['item_id'] = temp['item_id'].replace(self.data.item)
            Y_influenced = csr_matrix((np.ones(len(temp)), (temp['user_id'].values, temp['item_id'].values)), shape=(self.num_users, self.num_items), dtype=np.float32)
            Y_cut = Y - Y_influenced
            A8_cut = (Y_cut.dot(Y_cut.T)).multiply(B)
            A8 = A8 - A8_cut
            A9_cut = (Y_cut.dot(Y_cut.T)).multiply(U)
            A9_cut = A9_cut + A9_cut.T
            A9 = A9 - A9_cut
            A10 = A10 + A8_cut + A9_cut
