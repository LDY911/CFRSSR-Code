...
class GraphAugmentor(object):
    ...
    @staticmethod
    def edge_dropout(sp_adj, drop_rate):  # 工具函数，用于采样
        """Input: a sparse user-item adjacency matrix and a dropout rate."""
        adj_shape = sp_adj.get_shape()  # 得到交互矩阵形状
        edge_count = sp_adj.count_nonzero()  # 统计总边数
        row_idx, col_idx = sp_adj.nonzero()
        keep_idx = random.sample(range(edge_count), int(edge_count * (1 - drop_rate)))  # 采样70%
