...
class DiffNet(SocialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        if config.socail_network_improvement:  # whether to weight the social network
            print('Weighted type:', config.improvement_type, config.weighted_type)
            temp = torch.tensor(pickle.load(open('./dataset/' + dataset.dataset_name +'/'+dataset.dataset_name + '_weighted_' + str(config.file_type) + '.pkl', 'rb'))).to(self.device)
            if config.weighted_type == '0':
                self.net_edge_weight = temp[self.net_edge_index[1]].to(torch.float32)
            if config.weighted_type == '1':
                self.net_edge_weight_new = temp[self.net_edge_index[1]].to(torch.float32)
                self.net_edge_weight = (self.net_edge_weight + self.net_edge_weight_new) / 2
            if str(config.weighted_type) == '2':
                self.net_edge_weight_new = temp[self.net_edge_index[1]].to(torch.float32)
                self.net_edge_weight = self.net_edge_weight * self.net_edge_weight_new
                self.net_edge_weight = (self.net_edge_weight - self.net_edge_weight.min()) / (self.net_edge_weight.max() - self.net_edge_weight.min())
        ...
        self.socail_network_improvement = config.socail_network_improvement
        # self.predict_T = config.predict_T
    ...
    def get_norm_net_adj_mat(self, row, col, row_norm=False):
        edge_index = torch.stack([row, col])
        edge_index = edge_index.type(torch.int64).to(self.device)
        deg = degree(edge_index[0], self.n_users).to(self.device)

        if row_norm:
            norm_deg = 1. / torch.where(deg == 0, torch.ones([1]).to(self.device), deg)
            edge_weight = norm_deg[edge_index[0]]
        else:
            norm_deg = 1. / torch.sqrt(torch.where(deg == 0, torch.ones([1]).to(self.device), deg))
